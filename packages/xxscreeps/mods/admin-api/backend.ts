...
function parseBind(bind: string) {
	const addr: any[] = bind.split(':');
	addr[1] = Number(addr[1] ?? 21026);
	if (addr[0] === '*') {
		addr.shift();
	}
	addr.reverse();
...
hooks.register('backendReady', (db, shard) => {
	...
	for (const route of routes) {
		router[route.method ?? 'get'](route.path, async (context, next) => {
	...
	server.listen(...parseBind(bind), () => console.log('🔧 Admin API listening'));
});
