...
/** What one decoration type owes the renderer. */
interface Contract {
 ...
 */
export const washes: readonly { key: string; body: string }[] =
...
function requireProp(definition: DecorationDefinition, name: string) {
	...
	if (prop === undefined) {
		throw new Error(`Decoration '${definition._id}' is a '${definition.type}' decoration but declares no '${name}' property`);
	} else if (prop.default === undefined) {
		throw new Error(`Decoration '${definition._id}' seeds no default for '${name}', which the renderer reads out of every placement`);
	}
}
...
export function completeDefinition(definition: DecorationDefinition, assetUrl: (key: string) => string): DecorationDefinition {
	...
	for (const field of contract.fields) {
		if (definition[field] === undefined) {
			throw new Error(`Decoration '${definition._id}' is a '${definition.type}' decoration but declares no '${field}'`);
		}
	}
