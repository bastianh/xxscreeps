// Moves a whole server between storage providers: `archive export` walks every key of the database
// and of each configured shard into one file, `archive import` writes that file back. Registered as
// the `archive` subcommand; run after `tsc -b`: `xxscreeps archive <verb> <file>` (usage() lists
// them).
//
...
// build reading an archive back has to understand the blobs inside it, so restore onto the same
// version of xxscreeps or a newer one.
//
// Both ends want a stopped server. Export holds each shard's game mutex, so a server which is up
// but paused cannot tick mid-scan; import replaces the target's contents outright and has nothing
...
const magic = 'xxscreeps-archive 1\n';
...
/** Closes the file, so a truncated archive is caught instead of read as a short one. */
interface EndRecord {
	kind: 'end';
	entries: number;
}

type ArchiveRecord = EndRecord | EntryRecord | HeaderRecord;

interface Store {
	name: string;
	data: KeyValProvider;
	/**
	 * Per-tick state, which references the world it was built from. It is never exported, and a
...
function *frame(record: ArchiveRecord, payload?: Readonly<Uint8Array>) {
...
class ArchiveReader {
	...
	async read(size: number): Promise<Buffer> {
		return await this.readOptional(size) ?? function(): never {
			throw new Error('Archive ends mid-record');
		}();
	}

	/**
	 * Answers `undefined` where `read` would throw, for the one read which tolerates a stream
	 * ending early: the magic at the head of a file which may not be an archive at all. Trouble
	...
	async readRecord(): Promise<ArchiveRecord> {
		const length = (await this.read(4)).readUInt32BE();
		return JSON.parse((await this.read(length)).toString('utf8')) as ArchiveRecord;
	}
}

async function *readEntries(file: string, reader: ArchiveReader) {
	let count = 0;
	while (true) {
...
/** Reads an archive's header, along with an iterator over the entries behind it. */
async function openArchive(file: string) {
	const reader = new ArchiveReader(createReadStream(file));
	...
	if (opening?.toString('utf8') !== magic) {
		throw new Error(`'${file}' is not an xxscreeps archive`);
	}
 ...
 * The shards come along separately because export needs their mutexes.
 */
...
async function exportArchive(file: string) {
	await withStores(async (stores, shards) => {
		...
		// leave behind something that opens like a whole archive.
		const tmp = `${file}.tmp`;
		let entries = 0;
		await pipeline(async function*() {
			...
			for (const store of stores) {
				...
				for await (const [ key, type ] of scanAll(store.data)) {
					...
					// a login session -- and an archive has nowhere to put the time it has left, so
...
async function importArchive(file: string, force: boolean) {
	await withStores(async stores => {
		const byName = new Map(Fn.map(stores, store => [ store.name, store ] as const));
		const { entries: records, header } = await openArchive(file);
		...
		const sources = await Fn.mapAwait(header.stores, async source => {
			...
			if (target === undefined) {
				throw new Error(`Archive holds '${source.name}', which this config does not configure`);
			} else if (!source.separatesBlobs && await target.data.separatesBlobs()) {
				throw new Error(
					`'${source.name}' was exported from a provider which files text and binary together, ` +
		...
		// be filed into -- anything else would merge an archive into a store still holding a world
...
async function infoArchive(file: string) {
	const { entries, header } = await openArchive(file);
...
function usage(): never {
  ...
  archive export <file>          snapshot the database and every configured shard
  archive import <file> --force  replace those stores with the contents of an archive
  archive info   <file>          describe an archive without connecting to storage
`);
	process.exit(2);
}

const argv = checkArguments({ argv: true, boolean: [ 'force' ] as const });
const [ verb, file ] = argv.argv;
if (verb === undefined || file === undefined) {
	usage();
}
try {
	switch (verb) {
		case 'export': await exportArchive(file); break;
		case 'import': await importArchive(file, argv.force); break;
		case 'info': await infoArchive(file); break;
		default: usage();
	}
} catch (err) {
